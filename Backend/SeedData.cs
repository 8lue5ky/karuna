...
public class SeedData
{
    ...
    [
        new SeedUser()
        {
            Email = "leela@actioreact.io", 
            NormalizedEmail = "LEELA@ACTIOREACT.IO", 
        ...
        new SeedUser()
        {
            Email = "harry@actioreact.io",
            NormalizedEmail = "HARRY@ACTIOREACT.IO",
        ...
        new SeedUser()
        {
            Email = "kuppi.s90@gmail.com",
            NormalizedEmail = "KUPPI.S90@GMAIL.COM",
    ...
    public static async Task InitializeAsync(IServiceProvider serviceProvider)
    {
        ...
        {
            var hashed = password.HashPassword(user, passwords[i]);
            ...
            if (user.Email is not null)
            {
                ...
                if (appUser is not null && user.RoleList is not null)
                {
                    ...
                    if (i == 0)
                    {
                        var posts = CreatePosts(appUser).Where((item, index) => index % 2 == 0);

                        context.Posts.AddRange(posts);
                    } 
                    else if (i == 1)
                    {
                        var posts = CreatePosts(appUser).Where((item, index) => index % 2 == 1);
    ...
    private static Post[] CreatePosts(AppUser appUser)
    {
        Post[] posts =
        {
            new Post()
